  ...
  return (
    <div className="my-4 border-b border-b-300 py-8">
      <div className="mb-4">
        {author ? (
          <>
            Posté par: <span className="font-bold">{author}</span> le{" "}
            {formattedDate}
          </>
        ) : (
      ...
      {isEditable && (
        <div className="flex gap-3 font-bold py-2 px-4 rounded-md bg-slate-200 w-fit">
          <Link href={`/edit-post/${id}`}>Editer</Link>
          <DeleteButton id={id} />
        </div>
      )}
      ...
      {!isEditable && !pageId && (
        <Link
          className="bg-slate-400 w-fit text-white px-4 py-0.5 text-lg font-bold mt-4 block"
          href={`/posts/${id}`}
        >
          Lire
        </Link>
